@extends('layouts.app')
@section('title', 'Nova Transportadora - CalibSys')
@section('content')
...
<form action="{{ route('transportes.store') }}" method="POST" class="space-y-6">
    @csrf
    ...
    <x-card title="Dados da Transportadora" icon="fas fa-truck">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="transportadora" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" value="{{ old('transportadora') }}">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Motorista</label>
                <input type="text" name="motorista" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" value="{{ old('motorista') }}">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Veículo (Placa/Modelo)</label>
                <input type="text" name="veiculo" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" value="{{ old('veiculo') }}">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Documento de Transporte</label>
                <input type="text" name="documento_transporte" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" value="{{ old('documento_transporte') }}">
            </div>
            ...
            <div>
                ...
                <input type="email" name="email" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" value="{{ old('email') }}">
            </div>
    ...
    <div class="flex gap-3">
        <x-button type="submit" variant="primary" icon="fas fa-save">Salvar Transportadora</x-button>
