...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-3xl mx-auto">
            ...
            <form action="{{ route('lotes.store') }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="data_envio" id="data_envio" value="{{ old('data_envio', date('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="date" name="data_retorno" id="data_retorno" value="{{ old('data_retorno') }}"
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status"
                            ...
                            required>
                            <option value="preparacao" {{ old('status', 'preparacao') == 'preparacao' ? 'selected' : '' }}>Em
                                Preparação</option>
                            <option value="enviado" {{ old('status') == 'enviado' ? 'selected' : '' }}>Enviado</option>
                            <option value="em_calibracao" {{ old('status') == 'em_calibracao' ? 'selected' : '' }}>Em
                                Calibração</option>
