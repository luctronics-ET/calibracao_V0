        ...
        <div class="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg shadow-lg p-6 mb-6 text-white">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <p class="text-indigo-100 text-lg">{{ $equipamento->tipo }} - {{ $equipamento->fabricante ?? 'N/A' }}</p>
                    <p class="text-indigo-200 text-sm mt-1">Modelo: {{ $equipamento->modelo ?? 'N/A' }} | Série: {{ $equipamento->serie ?? 'N/A' }}</p>
                </div>
                ...
                <div class="bg-white shadow-lg rounded-xl p-5 border border-gray-100 hover:shadow-xl transition">
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="bg-blue-100 text-blue-700 px-3 py-1 rounded-full font-bold">{{ $equipamento->calibracoes->count() }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-4">
                ...
                @if($equipamento->foto)
                    ...
                    <div class="bg-white shadow-lg rounded-xl p-5 border border-gray-100 hover:shadow-xl transition">
                        ...
                        <img src="{{ asset('storage/' . $equipamento->foto) }}" alt="Foto" class="w-full rounded-lg shadow-md border border-gray-200">
                    </div>
                @endif
                ...
                <div class="bg-white shadow-lg rounded-xl p-5 border border-gray-100 hover:shadow-xl transition">
                    ...
                    @if($equipamento->calibracoes->count() > 0)
                        <div class="space-y-2">
                            @foreach($equipamento->calibracoes->sortByDesc('data_calibracao')->take(5) as $calibracao)
                                <div class="bg-gradient-to-br from-gray-50 to-gray-100 p-3 rounded-lg border border-gray-200 hover:border-emerald-300 transition">
                                    <div class="flex justify-between items-start mb-2">
                                        <span class="text-sm font-bold text-gray-700">{{ \Carbon\Carbon::parse($calibracao->data_calibracao)->format('d/m/Y') }}</span>
                                        @if($calibracao->resultado === 'aprovado')
                                            <span class="bg-green-500 text-white px-2 py-1 rounded-full text-xs font-semibold">✓ Aprovado</span>
                                        @elseif($calibracao->resultado === 'reprovado')
                                            <span class="bg-red-500 text-white px-2 py-1 rounded-full text-xs font-semibold">✗ Reprovado</span>
                                        @elseif($calibracao->resultado === 'condicional')
                                            <span class="bg-yellow-500 text-white px-2 py-1 rounded-full text-xs font-semibold">! Condicional</span>
                                        @endif
                                    </div>
                                    <div class="text-xs text-gray-600 space-y-1">
                                        <div class="flex items-center gap-1">
                                            ...
                                            Validade: <span class="font-medium">{{ \Carbon\Carbon::parse($calibracao->data_validade)->format('d/m/Y') }}</span>
                                        </div>
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <p class="text-gray-400 text-sm">Nenhuma calibração registrada</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="bg-white shadow-lg rounded-xl p-5 border border-gray-100 hover:shadow-xl transition">
                    ...
                    @if($equipamento->loteItens->count() > 0)
                        <div class="space-y-2">
                            @foreach($equipamento->loteItens->sortByDesc('loteEnvio.data_envio')->take(5) as $item)
                                <div class="bg-gradient-to-br from-orange-50 to-yellow-50 p-3 rounded-lg border border-orange-200 hover:border-orange-400 transition">
                                    <div class="flex justify-between items-start mb-2">
                                    ...
                                    <div class="text-xs text-gray-600 space-y-1">
                                        <div class="flex items-center gap-1">
                                            ...
                                            Envio: <span class="font-medium">{{ \Carbon\Carbon::parse($item->loteEnvio->data_envio)->format('d/m/Y') }}</span>
                                        </div>
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <p class="text-gray-400 text-sm">Não incluído em lotes</p>
                        </div>
                    @endif
                </div>
